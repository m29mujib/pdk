...
// 🎯 Aksi tabel (dikirim ke reusable component)
const tableActions = [
  {
    key: 'edit',
    label: 'Edit',
...
const onEdit = (record: Jadwal) => {
  message.info(`✏️ Edit jadwal: ${record.judul}`)
}

const onDelete = (record: Jadwal) => {
  message.warning(`🗑️ Hapus jadwal: ${record.judul}`)
}
...
const handleSubmit = () => {
  console.log('📤 Broadcast:', form.value)
